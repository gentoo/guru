...
MOZ_FIREFOX_FILE="mullvad-browser"

if [[ ! -r ${MOZ_LIB_DIR}/mullvad-browser/${MOZ_FIREFOX_FILE} ]]; then
	if [[ ! -r ${SECONDARY_LIB_DIR}/mullvad-browser/${MOZ_FIREFOX_FILE} ]]; then
		echo "Error: ${MOZ_LIB_DIR}/mullvad-browser/${MOZ_FIREFOX_FILE} not found" >&2
		if [[ -d ${SECONDARY_LIB_DIR} ]]; then
			echo "       ${SECONDARY_LIB_DIR}/mullvad-browser/${MOZ_FIREFOX_FILE} not found" >&2
		fi
		exit 1
	fi
	MOZ_LIB_DIR="${SECONDARY_LIB_DIR}"
fi
MOZILLA_FIVE_HOME="${MOZ_LIB_DIR}/mullvad-browser"
MOZ_EXTENSIONS_PROFILE_DIR="${HOME}/.mullvad-browser/extensions/{ec8030f7-c20a-464f-9b0e-13a3a9e97384}"
...
if [[ -n "${MOZ_PLUGIN_PATH}" ]]; then
	MOZ_PLUGIN_PATH=${MOZ_PLUGIN_PATH}:${MOZ_LIB_DIR}/mullvad-browser/${MOZ_PLUGIN_DIR}
else
	MOZ_PLUGIN_PATH=${MOZ_LIB_DIR}/mullvad-browser/${MOZ_PLUGIN_DIR}
fi

if [[ -d "${SECONDARY_LIB_DIR}/mullvad-browser/${MOZ_PLUGIN_DIR}" ]]; then
	MOZ_PLUGIN_PATH=${MOZ_PLUGIN_PATH}:${SECONDARY_LIB_DIR}/mullvad-browser/${MOZ_PLUGIN_DIR}
fi
