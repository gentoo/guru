...
UBLOCK_PV="1.72.2"
UBLOCK_XPI="uBlock0_${UBLOCK_PV}.firefox.signed.xpi"
...
src_prepare() {
	...
	sed -i 's/#\  ifndef/#ifndef/g' "${S}/toolkit/content/jar.mn" || die
	sed -i 's/#\  endif/#endif/g' "${S}/toolkit/content/jar.mn" || die

	# ValueError: Item already in manifest: chrome/toolkit/content/global/aboutTelemetry.xhtml
	# this is caused because our mullvad branding conflicts with the  original  aboutTelemetry
	# in order to get around this, we remove the first entry and keep the  mullvad branded one
	sed -i '/aboutTelemetry.xhtml/c\' "./toolkit/content/jar.mn" \
	|| die "could not remove original 'about' telemetry file"
...
src_configure() {
	...
	mozconfig_add_options_ac 'Gentoo default' \
		...
		--with-base-browser-version="${PV}"

	# Mullvad
	mozconfig_add_options_ac 'Mullvad Branding' \
		--with-app-name="mullvad-browser" \
		--with-app-basename="mullvad-browser" \
		--with-branding=browser/branding/mb-release
	export MOZ_APP_REMOTINGNAME=Mullvad
...
src_install() {
	...
	# Install MULLVAD BROWSER default extensions ( mullvad, ublock )
	# these are normally provided when getting the pre-built version
	moz_install_xpi "${MOZILLA_FIVE_HOME}/browser/extensions" \
	"${DISTDIR}/${MULLVAD_EXT_XPI}" \
	"${DISTDIR}/${UBLOCK_XPI}"
	...
	# Install default mullvad policies
	insinto "${MOZILLA_FIVE_HOME}/distribution"
	newins "${FILESDIR}"/distribution.ini distribution.ini
	newins "${FILESDIR}"/mullvad-policies.json policies.json

	# Install system-wide preferences including extension prefs
	...
	# Install icons
	local icon_srcdir="${S}/browser/branding/mb-release"
	...
	# Install menu
	local app_name="Mullvad Browser"
