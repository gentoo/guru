...
PATCHES=(
  "${FILESDIR}/${PN}-0.14.2-use-GNUInstallDirs.patch"
)
...
src_prepare() {
  # Fix CRLF and missing EOF newline to satisfy strict patch application
  sed -i 's/\r$//' CMakeLists.txt
  [ -n "$(tail -c1 CMakeLists.txt)" ] && echo >>CMakeLists.txt
