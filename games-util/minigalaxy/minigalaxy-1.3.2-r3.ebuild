...
src_install() {
	...
	mkdir -p usr/share/{minigalaxy,metainfo} || die
	cp -R data/images data/ui data/style.css usr/share/minigalaxy || die
	cp data/io.github.sharkwouter.Minigalaxy.metainfo.xml usr/share/metainfo || die
	insinto /usr
	doins -r usr/share
	...
	local lo
	for lo in data/po/*.po; do
		msgfmt "${lo}" -o "${lo%.po}.mo" && domo "${lo%.po}".mo || die
	done
