...
src_prepare() {
	...
	sed -i 's/\<citra\([^-_\/\.A-Za-z]\)/citra-canary\1/g' src/citra/CMakeLists.txt CMakeLists.txt || die
	sed -i 's/citra$/&-canary/g' src/citra/CMakeLists.txt || die
	sed -i 's/citra/&-canary/g' dist/citra.{desktop,xml} || die
	for file in dist/citra*; do
		mv ${file} ${file/citra/citra-canary}
	done
	sed -i '/citra-room[^\.]/s/citra/&-canary/g' src/dedicated_room/CMakeLists.txt || die
	sed -i '/citra-room\./s/citra/&-canary/g' src/dedicated_room/citra-room.cpp || die
	sed -i 's/citra-room$/citra-canary-room/' src/dedicated_room/CMakeLists.txt || die

	sed -i '/citra-qt/s/citra/&-canary/g' CMakeModules/CopyCitraQt5Deps.cmake CMakeLists.txt || die
	sed -i '/citra-qt[^\.]/s/citra/&-canary/g' src/citra_qt/CMakeLists.txt || die
	sed -i 's/citra-qt$/citra-canary-qt/' src/citra_qt/CMakeLists.txt || die

	sed -i 's/copy_citra-canary/copy_citra_canary/' src/citra_qt/CMakeLists.txt || die
