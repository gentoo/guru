...
EXAMPLES_GGUF="stories15M-q4_0.gguf"
EXAMPLES_GGUF_DIST="${PN}-${EXAMPLES_GGUF}"
...
SRC_URI="
	...
	examples? ( https://huggingface.co/ggml-org/models/resolve/main/tinyllamas/stories15M-q4_0.gguf?download=true -> ${PN}-${EXAMPLES_GGUF} )
"
...
IUSE="curl openblas +openmp blis hip cuda opencl vulkan flexiblas wmma examples"
...
src_prepare() {
	...
	cmake_src_prepare
	use examples && mkdir -p "${BUILD_DIR}/tinyllamas" && cp "${DISTDIR}/${EXAMPLES_GGUF_DIST}" "${BUILD_DIR}/tinyllamas/${EXAMPLES_GGUF}"
}
