...
#		bin/lldb/linux/x64/lib/python3.12
	...
	for file in "plugins/jcef-plugin/jcef"/{libjcef.so,jcef_helper}
	do
		if [[ -f "${file}" ]]; then
...
src_install() {
	...
	# lib files
	for _f in jexec jspawnhelper; do
		fperms 755 "${DIR}/${JRE_DIR}/lib/$_f"
	done

	# jcef files
	for _f in chrome-sandbox jcef_helper; do
		fperms 755 "${DIR}/plugins/jcef-plugin/jcef/$_f"
	done
